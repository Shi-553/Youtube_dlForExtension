...
(async () => {
    const userPofile = await browser.runtime.sendMessage({ isGetUserPofile: true });
    if (setMainDownloadDirectory.value == "")
        setMainDownloadDirectory.value = userPofile + "\\Downloads";
    if (setSubDownloadDirectory.value == "")
    setSubDownloadDirectory.value = userPofile + "\\Videos";


    const mainDownloadDirectoryUpdate = async () => {
        if (setMainDownloadDirectory.value == "")
            setMainDownloadDirectory.value = userPofile + "\\Downloads";
    ...
    const subDownloadDirectoryUpdate = async () => {
        if (setSubDownloadDirectory.value == "")
            setSubDownloadDirectory.value = userPofile + "\\Videos";
...
savePreset.addEventListener("click", async () => {
    ...
    if (option.preset[selectedKey].isShareJson != setIsShareJson.checked ||
        ...
        browser.runtime.sendMessage({ changePresetValue: true, key: selectedKey, isShareJson: setIsShareJson.checked });
    }
